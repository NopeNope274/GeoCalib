...
# gr.File(file_count="multiple") requires a list of files per example
example_images = [
...
def process_results(image_paths, camera_model_korean, plot_up, plot_up_confidence, plot_latitude, plot_latitude_confidence, plot_undistort, progress=gr.Progress(track_tqdm=False)):
    ...
    for i, single_image_path in enumerate(image_paths):
        ...
        # 메모리 최적화를 위해 원본 이미지는 제외하고 텐서 추론 데이터만 리스트에 보관합니다.
    ...
    return txt, plot_img, all_inference_results, avg_vfov_deg, avg_hfov_deg, orig_w, orig_h, slider_update
...
def render_frame(frame_idx, all_results, plot_up, plot_up_confidence, plot_latitude, plot_latitude_confidence, plot_undistort):
    ...
    # User-facing slider is 1-indexed
    res = all_results[int(frame_idx) - 1]
    
    # 필요한 순간에만 이미지를 텐서화하여 가져옵니다 (메모리 최적화)
    img_tensor = model.load_image(res["image_path"])
    
    # update_plot 이 바라보는 호환 형식 생성
    temp_res = {
        "camera": res["camera"],
        "gravity": res["gravity"],
...
# ----------------- 7. UI Definition ----------------- 
with gr.Blocks(title="GeoCalib for MBC Design Team", css=custom_css) as demo:
    ...
    submit_btn.click(
        ...
        outputs=[text_output, image_output, state_all_results, state_vfov, state_hfov, state_orig_w, state_orig_h, frame_slider],
    )
